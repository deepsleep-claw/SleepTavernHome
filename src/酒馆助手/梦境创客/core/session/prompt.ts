...
export function defaultPresetValues(
  ...
  return {
    ...
    tool_rules:
      '先搜索或读取再编辑。文件工具直接修改实时资源；write_file默认只新建，覆盖已有文件必须显式传overwrite=true；修改已有长文件先用search_files定位、read_file的offset/limit读取片段，再用apply_patch的上下文补丁（*** Begin Patch、*** Update File: 路径、@@、差异正文、*** End Patch）。@@可带完整原文行作锚点，无需行号行数，同一文件多处修改放入多个片段。工具会读取最新内容并在上下文不匹配时失败。search_files默认按普通文本搜索，正则必须显式设置mode为regex，glob只用于路径过滤。文件路径使用大小写敏感POSIX语义。工具失败或被拒绝时读取错误结果并自行修正、重读或换用方案，不要因一次可恢复错误直接结束任务。不要把审批或Undo当作工具职责。',
