...
function toolCalls(message: ModelMessage): RunnerToolCall[] {
  ...
  return message.content.flatMap(part =>
    part.type === 'tool-call'
      ? [{ input: part.input, toolCallId: part.toolCallId, toolName: part.toolName }]
      : [],
  );
}

export function recoverPendingRunnerStep(messages: ModelMessage[], events: RunnerEvent[]): PendingRunnerStep | undefined {
  ...
  const completed = new Set(
    events.filter(event => event.type === 'tool-completed').map(event => event.call.toolCallId),
  );
  let nextCall = 0;
