...
describe('RemoteBuiltinSkillStore', () => {
  ...
  it('从脚本同级资源清单下载、校验并缓存一个自定义后缀ZIP，之后可离线从酒馆文件加载', async () => {
    ...
    const store = new RemoteBuiltinSkillStore(
    ...
    expect(settings.load().builtinSkillPackages['html-project']).toMatchObject({
      sha256: hash,
      size: bytes.byteLength,
    });
    ...
    const updatedBytes = exportSkillZip({ ...skill, body: '更新后的正文' });
    manifest.skills[0].sha256 = await sha256(updatedBytes);
    manifest.skills[0].size = updatedBytes.byteLength;
    fetchMock.mockImplementation(async (input: RequestInfo | URL) =>
      String(input).endsWith('manifest.json')
        ? new Response(JSON.stringify(manifest))
        : new Response(Uint8Array.from(updatedBytes)),
    );
    await store.refreshManifest();
    expect((await store.ensure('html-project')).body).toBe('更新后的正文');
  });
