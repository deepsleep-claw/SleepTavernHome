...
- 新建条目可以直接向 `entries/` 写入一个不重名的 `.md` 文件，不要自行编造 UID。提交后系统会分配正式 UID 并规范化文件名；`write_file` 返回的 `path` 为写后真实路径，`mutation.paths` 列出本次变更路径。
- 删除条目后，后续条目的顺序号会重新编号。当前会话可按稳定资源身份继续解析已经见过的旧路径；后续读取和修改优先使用工具返回的最新路径。
