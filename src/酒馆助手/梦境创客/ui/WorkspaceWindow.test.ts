    ...
    builtinSkillResources: [
      {
        ...
        sha256: 'hash-html',
        size: 2048,
        sourceUrl: 'http://127.0.0.1:5500/resources/html-project.zip.ds',
        state: 'available',
        version: 1,
      },
      {
        ...
        file: 'tavern-helper-api.zip.ds',
        id: 'tavern-helper-api',
        loading: 'on-demand',
        name: '酒馆助手API参考',
        sha256: 'hash-api',
        size: 4096,
        sourceUrl: 'http://127.0.0.1:5500/resources/tavern-helper-api.zip.ds',
        state: 'missing',
        version: 1,
      },
    ],
...
describe('WorkspaceWindow', () => {
  ...
  it('默认隐藏Revision和侧栏，并折叠连续工具与思考过程', async () => {
    ...
    expect(root.querySelector('.dca-version-tab')?.textContent).toContain('v0.2.0');
    ...
    updateSection.click();
    await nextTick();
    expect(root.querySelector('.dca-update-settings')?.textContent).toContain('当前版本 v0.2.0');
