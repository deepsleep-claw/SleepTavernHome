...
describe('floating button anchor', () => {
  it('保留自由位置，并按照左右与中心锚点响应宽度变化', () => {
    ...
    const rightAnchor = nearestFloatingButtonAnchor({ x: rightPosition.x + 25, y: rightPosition.y + 25 }, viewport);
    ...
    const leftAnchor = nearestFloatingButtonAnchor({ x: leftPosition.x + 25, y: leftPosition.y + 25 }, viewport);
    ...
    const centerAnchor = nearestFloatingButtonAnchor({ x: centerPosition.x + 25, y: centerPosition.y + 25 }, viewport);
