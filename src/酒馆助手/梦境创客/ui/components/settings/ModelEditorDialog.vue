<template>
  <div v-if="open" class="dca-modal-backdrop" role="presentation">
    <section class="dca-modal dca-model-editor" role="dialog" aria-modal="true" aria-labelledby="dca-model-editor-title">
      <header>
        <div>
      ...
      <div v-else class="dca-model-editor-body">
        ...
        <section class="dca-model-template-box">
          ...
          <div class="dca-template-picker">
            <SearchableCombobox v-model="selectedTemplateId" :options="templateOptions" placeholder="搜索本地或云端模板" />
        ...
        <div class="dca-form-grid" @input.capture="markCustom" @change.capture="markCustom">
          <label class="dca-field"><span>上下文窗口</span><input v-model.number="form.contextWindow" min="0" type="number" /><small>0 表示自动。</small></label>
          <label class="dca-field"><span>输出上限</span><input v-model.number="form.maxOutputTokens" min="0" type="number" /><small>0 表示不主动发送限制。</small></label>
          <label class="dca-field" :class="{ muted: samplingIgnored }">
            <span>温度 <i v-if="samplingIgnored" class="fa-solid fa-circle-exclamation" title="当前模式会保留但不发送此参数。"></i></span>
            <input v-model.number="form.temperature" min="0" step="0.01" type="number" placeholder="自动" />
          </label>
          <label class="dca-field" :class="{ muted: samplingIgnored }">
            <span>Top P <i v-if="samplingIgnored" class="fa-solid fa-circle-exclamation" title="当前模式会保留但不发送此参数。"></i></span>
          ...
          <label class="dca-field wide">
            <span>推理强度</span>
            <textarea v-model="form.reasoningEfforts" rows="4" spellcheck="false" placeholder="低:low&#10;高:high"></textarea>
          </label>
        </div>
        ...
        <details class="dca-advanced-request">
          ...
          <div class="dca-form-grid">
            <label class="dca-field wide">
              <span>自定义请求标头（JSON）</span>
              <textarea v-model="form.headers" rows="5" spellcheck="false" placeholder="{}"></textarea>
            </label>
            <label class="dca-field">
              <span>附加参数格式</span>
              <DcaSelect
                aria-label="模型附加参数格式"
                :model-value="form.extraFormat"
                :options="extraFormatOptions"
                @update:model-value="convertExtraFormat"
              />
            </label>
            <label class="dca-field wide">
              <span>自定义附加参数</span>
              <textarea v-model="form.extraText" rows="8" spellcheck="false" placeholder="reasoning:&#10;  effort: high"></textarea>
              <small>null 可删除 Provider 层的同名字段；受保护的模型、消息、工具和流式字段不会被覆盖。</small>
            </label>
          </div>
        </details>
      </div>
...
type Form = {
  ...
  enabled: boolean;
  extraFormat: AdvancedRequestFormat;
  extraText: string;
  headers: string;
  id: string;
...
const extraFormatOptions: SelectOption[] = [
...
const availableTemplates = computed(() => filterModelTemplatesForScope(allTemplates.value, {
...
const templateSource = computed(() => selectedTemplate.value?.source === 'cloud' ? 'models.dev' : selectedTemplate.value ? '本地配置' : '自定义');
const samplingIgnored = computed(() => form.compatibilityMode === 'deepseek');

watch(() => [props.open, props.model?.id, props.copying] as const, async ([open]) => {
  if (!open) return;
  loading.value = true;
  applying = true;
  autoApplied = false;
  try {
    ...
    if (props.model) {
      ...
      Object.assign(form, {
        ...
        extraFormat: request?.extraParameters.format ?? 'yaml',
        extraText: request?.extraParameters.text ?? '',
...
watch(() => form.modelId, modelId => {
  if (applying || form.id || autoApplied || !modelId.trim()) return;
  const match = matchModelTemplates(modelId, builtins.filter(item => !item.interfaceType || item.interfaceType === props.provider?.interfaceType), 1)[0];
  if (!match) return;
...
function emptyForm(): Form {
  ...
  return {
    capabilities: { ...settings.capabilities }, compatibilityMode: 'standard', contextWindow: 0, enabled: true,
    extraFormat: 'yaml', extraText: '', headers: '{}', id: '', maxOutputTokens: 0, modelId: '', name: '',
    reasoningEfforts: '', temperature: '', topP: '',
  };
}
...
function formatEfforts(value: Array<{ id: string; name: string }>) { return value.map(item => `${item.name}:${item.id}`).join('\n'); }
function optionalNumber(value: number | string) { return typeof value === 'number' && Number.isFinite(value) ? value : undefined; }

function currentSettings() {
  return {
    capabilities: { ...form.capabilities }, contextWindow: form.contextWindow, maxOutputTokens: form.maxOutputTokens,
    reasoningEfforts: parseEfforts(form.reasoningEfforts), temperature: optionalNumber(form.temperature), topP: optionalNumber(form.topP),
  };
}
...
function applyTemplate(template: ModelTemplate) {
  ...
  Object.assign(form, {
    capabilities: { ...settings.capabilities }, compatibilityMode: template.compatibilityMode ?? form.compatibilityMode,
    contextWindow: settings.contextWindow, maxOutputTokens: settings.maxOutputTokens,
    reasoningEfforts: formatEfforts(settings.reasoningEfforts), temperature: settings.temperature ?? '', topP: settings.topP ?? '',
  });
  appliedTemplate.value = { id: template.id, revision: template.revision, source: template.source };
  queueMicrotask(() => { applying = false; });
}
function applySelectedTemplate() { if (selectedTemplate.value) applyTemplate(selectedTemplate.value); }
function markCustom() { if (!applying) { selectedTemplateId.value = 'custom'; appliedTemplate.value = undefined; } }

async function loadCloud() {
  cloudLoading.value = true;
  try { clouds.value = await fetchModelsDevCatalog(); toastr.success(`读取了 ${clouds.value.length} 个云端模型模板。`, '梦境创客'); }
  catch (error) { toastr.warning(error instanceof Error ? error.message : String(error), '梦境创客'); }
  finally { cloudLoading.value = false; }
}

async function convertExtraFormat(value: string) {
  if (value !== 'json' && value !== 'yaml') return;
  try {
    const converted = convertAdvancedRequestDocument({ format: form.extraFormat, text: form.extraText }, value);
    form.extraFormat = converted.format; form.extraText = converted.text;
  } catch (error) { toastr.error(error instanceof Error ? error.message : String(error), '梦境创客'); }
}
function updateCompatibility(value: string) { if (value === 'standard' || value === 'deepseek') form.compatibilityMode = value; }
function updateCapability(key: ModelCapabilityKey, value: string) { if (value === 'auto' || value === 'enabled' || value === 'disabled') form.capabilities[key] = value; }

async function save() {
  if (!props.provider) return;
  let headers: Record<string, string>;
  try { headers = form.headers.trim() ? JSON.parse(form.headers) as Record<string, string> : {}; }
  catch { toastr.error('模型自定义请求标头不是有效的JSON。', '梦境创客'); return; }
  ...
  const succeeded = await action(async () => {
    saved = await runtime.saveModel(props.provider!.id, {
      ...
      extraParameters: { format: form.extraFormat, text: form.extraText },
      headers,
...
<style lang="scss">
.dca-model-editor { width: min(58rem, calc(100vw - 2rem)); max-height: min(90vh, 54rem); }
.dca-model-editor-body { display: flex; min-height: 0; flex-direction: column; gap: .8rem; overflow-y: auto; padding: .1rem; }
.dca-model-template-box { display: flex; flex-direction: column; gap: .5rem; border: 1px solid var(--dca-border); border-radius: var(--dca-radius-md); padding: .7rem; background: var(--dca-raised); }
.dca-model-template-box > header { display: flex; align-items: center; justify-content: space-between; gap: .5rem; }
.dca-model-template-box > header > div { display: flex; flex-direction: column; }
.dca-model-template-box small, .dca-model-template-box > header span { color: var(--dca-text-muted); font-size: .72rem; }
.dca-model-editor .dca-template-picker { display: grid; grid-template-columns: minmax(0, 1fr) auto auto; gap: .4rem; }
.dca-inline-check { display: flex; flex-direction: row; align-items: center; gap: .45rem; }
.dca-inline-check input { width: auto; }
.dca-model-editor .muted { color: var(--dca-text-muted); }
.dca-advanced-request { border: 1px solid var(--dca-border); border-radius: var(--dca-radius-md); padding: .65rem; }
.dca-advanced-request summary { cursor: pointer; font-weight: 600; }
.dca-advanced-request[open] summary { margin-bottom: .7rem; }
@media (max-width: 720px) {
  .dca-model-editor { width: 100%; height: 100%; max-height: none; border-radius: 0; }
  .dca-model-editor .dca-template-picker { grid-template-columns: 1fr 1fr; }
  .dca-model-editor .dca-template-picker > :first-child { grid-column: 1 / -1; }
}
</style>
